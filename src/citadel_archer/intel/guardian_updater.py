...
def rules_from_ioc(item: IntelItem) -> List[GuardianRule]:
    ...
    if ioc.ioc_type in hash_types:
        rules.append(GuardianRule(
            ...
            indicator=ioc.value,
            severity=sev,
            action=action if _SEVERITY_RANK[sev] >= _SEVERITY_RANK[RuleSeverity.HIGH] else RuleAction.ALERT,
        ...
        rules.append(GuardianRule(
            ...
            indicator=ioc.value,
            severity=sev,
            action=RuleAction.BLOCK if _SEVERITY_RANK[sev] >= _SEVERITY_RANK[RuleSeverity.MEDIUM] else RuleAction.ALERT,
